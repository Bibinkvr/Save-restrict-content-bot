...
from pyrogram.types import Message, BotCommand, BotCommandScopeDefault, BotCommandScopeChat
...
LOGO = r"""
   _____            _             _    _____                     
  / ____|          | |           | |  / ____|                    
 | |     ___  _ __ | |_ ___ _ __ | |_| (___   __ ___   _____ _ __ 
 | |    / _ \| '_ \| __/ _ \ '_ \| __|\___ \ / _` \ \ / / _ \ '__|
 | |___| (_) | | | | ||  __/ | | | |_ ____) | (_| |\ V /  __/ |   
  \_____\___/|_| |_|\__\___|_| |_|\__|_____/ \__,_| \_/ \___|_|   
                     ⚡ 𝙱𝙾𝚃 𝙾𝙽𝙻𝙸𝙽𝙴 ⚡
"""
...
class Bot(Client):
    ...
    async def set_bot_commands_list(self):
        user_commands = [
            BotCommand("start", "Start the bot & dashboard"),
            BotCommand("help", "Help guide & instructions"),
            BotCommand("settings", "Interactive settings panel"),
            BotCommand("commands", "List all bot commands"),
            BotCommand("login", "Login Telegram account"),
            BotCommand("logout", "Disconnect Telegram session"),
            BotCommand("cancel", "Cancel current download/task"),
            BotCommand("myplan", "Check current plan & quota"),
            BotCommand("premium", "Upgrade to Premium"),
            BotCommand("setchat", "Set target forward/dump chat"),
            BotCommand("set_thumb", "Set custom thumbnail"),
            BotCommand("view_thumb", "View current thumbnail"),
            BotCommand("del_thumb", "Delete custom thumbnail"),
            BotCommand("thumb_mode", "Check thumbnail mode"),
            BotCommand("set_caption", "Set custom caption template"),
            BotCommand("see_caption", "View current custom caption"),
            BotCommand("del_caption", "Delete custom caption"),
            BotCommand("set_del_word", "Add words to delete"),
            BotCommand("rem_del_word", "Remove delete words"),
            BotCommand("set_repl_word", "Add word replacement rule"),
            BotCommand("rem_repl_word", "Remove replacement rule"),
        ]

        admin_commands = user_commands + [
            BotCommand("broadcast", "[Admin] Broadcast message"),
            BotCommand("users", "[Admin] Registered users count"),
            BotCommand("add_premium", "[Admin] Grant premium access"),
            BotCommand("remove_premium", "[Admin] Revoke premium access"),
            BotCommand("ban", "[Admin] Ban user from bot"),
            BotCommand("unban", "[Admin] Unban user"),
            BotCommand("set_dump", "[Admin] Set user dump chat"),
            BotCommand("add_fsub", "[Admin] Add FSub channel"),
            BotCommand("del_fsub", "[Admin] Remove FSub channel"),
            BotCommand("fsub", "[Admin] FSub channel status"),
            BotCommand("auto_approve", "[Admin] Toggle auto-approval"),
        ]
        ...
        try:
            ...
            for admin_id in ADMINS:
                try:
                    ...
                    pass
            logger.info("Bot commands list updated successfully.")
        except Exception as e:
